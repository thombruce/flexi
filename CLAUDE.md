...
The duplication is **deliberate** (rule of three). Now that a third tool exists, the clearest extraction candidate is the config-discovery + atomic-write plumbing (repeated 3×), **not** duration parsing (only 2×). Extract when touching it next becomes a chore, not speculatively.
